...
import { Activity, TrendingUp, UserCheck, Users, UserX } from "lucide-react"
...
function actionLabel(action: string): { label: string; type: string } {
  ...
  if (a === "LOGOUT") return { label: "SesiÃ³n cerrada", type: "logout" }
  if (a === "LOGIN") return { label: "Inicio de sesiÃ³n", type: "login" }
  if (a === "TOKEN_REFRESH") return { label: "RenovaciÃ³n de sesiÃ³n", type: "refresh" }
  if (a === "CONFIG_UPDATE") return { label: "ConfiguraciÃ³n actualizada", type: "config" }
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">Bienvenido al panel de administraciÃ³n</p>
      </div>
      ...
      <div className="bg-card/80 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Ãšltimas acciones en el sistema</p>
          </div>
        </div>
        <div className="space-y-4">
          {logs.length === 0 ? (
            ...
            logs.map((entry) => {
              ...
              const who = entry.targetEmail ?? entry.actorEmail ?? "â€”"
              return (
                <div
                  key={entry.id}
